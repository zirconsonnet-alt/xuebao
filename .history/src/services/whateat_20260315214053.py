...
class WhateatService(BaseService):
    ...
    async def recommend_food_tool(self, event: GroupMessageEvent) -> dict:
        if not self.enabled:
            return {"success": False, "message": "吃喝推荐服务未开启"}
        await self.recommend_food(event)
    ...
    async def recommend_drink_tool(self, event: GroupMessageEvent) -> dict:
        if not self.enabled:
            return {"success": False, "message": "吃喝推荐服务未开启"}
        await self.recommend_drink(event)
