...
from .base import BaseService, ServiceConfigOption, check_enabled, config_property, service_action
...
def _get_full_history_range(now: datetime) -> Tuple[datetime, datetime]:
    if now.tzinfo is not None:
        start = datetime.fromtimestamp(0, tz=now.tzinfo)
    else:
...
class WordcloudService(BaseService):
    ...
    settings_schema = [
        {"key": "daily_schedule_enabled", "title": "每日定时发送", "description": "控制是否每天自动发送群词云。", "type": "bool", "group": "自动化"},
        {"key": "daily_schedule_time", "title": "每日发送时间", "description": "设置每日自动发送群词云的时间。", "type": "time", "group": "自动化", "placeholder": "请输入 HH:MM，例如 22:00"},
    ]
    ...
    async def _send_wordcloud(self, *, event: Optional[GroupMessageEvent], start: datetime, stop: datetime, scope: str, user_id: Optional[int] = None):
        ...
        if image:
            receipt = await saa.Image(image).send_to(target)
            from src.services._ai.message_bridge import record_group_media_output

            scope_label = "个人" if scope == "personal" else "群聊"
            record_group_media_output(
                self.group.group_id,
                text=f"已发送{scope_label}词云图片",
    ...
    @service_action(
        cmd="今日词云",
        aliases={"本群今日词云", "我的今日词云"},
        desc="生成今日词云",
        tool_callable=True,
        record_ai_context=True,
        ai_context_label="生成今日词云",
    )
    @check_enabled
    ...
    @service_action(
        cmd="昨日词云",
        aliases={"本群昨日词云", "我的昨日词云"},
        desc="生成昨日词云",
        tool_callable=True,
        record_ai_context=True,
        ai_context_label="生成昨日词云",
    )
    @check_enabled
    ...
    @service_action(
        cmd="本周词云",
        aliases={"本群本周词云", "我的本周词云"},
        desc="生成本周词云",
        tool_callable=True,
        record_ai_context=True,
        ai_context_label="生成本周词云",
    )
    @check_enabled
    ...
    @service_action(
        cmd="上周词云",
        aliases={"本群上周词云", "我的上周词云"},
        desc="生成上周词云",
        tool_callable=True,
        record_ai_context=True,
        ai_context_label="生成上周词云",
    )
    @check_enabled
    ...
    @service_action(
        cmd="本月词云",
        aliases={"本群本月词云", "我的本月词云"},
        desc="生成本月词云",
        tool_callable=True,
        record_ai_context=True,
        ai_context_label="生成本月词云",
    )
    @check_enabled
    ...
    @service_action(
        cmd="上月词云",
        aliases={"本群上月词云", "我的上月词云"},
        desc="生成上月词云",
        tool_callable=True,
        record_ai_context=True,
        ai_context_label="生成上月词云",
    )
    @check_enabled
    ...
    @service_action(
        cmd="年度词云",
        aliases={"本群年度词云", "我的年度词云"},
        desc="生成年度词云",
        tool_callable=True,
        record_ai_context=True,
        ai_context_label="生成年度词云",
    )
    @check_enabled
    ...
    @service_action(
        cmd="历史词云",
        aliases={"本群历史词云", "我的历史词云"},
        need_arg=True,
        desc="生成指定时间段词云（支持 ISO8601 或 开始~结束，留空表示全部历史）",
        tool_callable=True,
        record_ai_context=True,
        ai_context_label="生成历史词云",
    ...
    async def wordcloud_history(self, event: GroupMessageEvent, arg: Message):
        ...
        if not raw_arg:
            start, stop = _get_full_history_range(now)
        else:
            try:
                start, stop = _parse_wordcloud_history_range(now, raw_arg)
            except ValueError as exc:
                message = f"❌ {exc}\n示例：/历史词云 2026-02-01~2026-02-05"
                await self.group.send_msg(message)
                return {"success": False, "message": message}
        await self._send_wordcloud(event=event, start=start, stop=stop, scope=self._resolve_scope(event))
        return {"success": True, "message": "已执行: 历史词云"}
    ...
    def _ensure_daily_schedule_runtime(self) -> None:
        ...
        self._sync_daily_schedule_state(
            time_str=self.daily_schedule_time or "22:00",
            callback_id=callback_id,
            enabled=bool(self.daily_schedule_enabled),
        )
    ...
    def on_config_option_changed(
        ...
        if option.key in {"daily_schedule_enabled", "daily_schedule_time"}:
            self._ensure_daily_schedule_runtime()
