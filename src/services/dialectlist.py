...
class DialectlistService(BaseService):
    ...
    @service_action(cmd="B话榜服务")
    async def dialectlist_service_menu(self):
        if not self.enabled:
            await self.group.send_msg("❌ B话榜服务未开启！")
            return
        routes = {
            "1": self.rank,
            "2": self.kkb,
        ...
        flow = {
            ...
            "badges": [
                ...
                {"text": "榜单查看", "tone": "success"},
            ],
            "sections": [
                {
                    "title": "可用操作",
                    "description": "一个入口看排行，一个入口看示例内容。",
                    "columns": 1,
                    "items": [
                        {
                            "index": "1",
                            "title": "B话榜",
                            "description": "查看当前群的 B 话排行。",
                            "meta": "回复 1 执行",
                            "status": "排行",
                            "status_tone": "success",
                        },
