...
class InfoService(BaseService):
    ...
    @ai_tool(
        name="get_user_info",
        desc="获取指定用户或当前说话用户的详细信息；在群聊上下文下会一并返回该用户的群成员信息",
    ...
    async def get_user_info_core(self, user_id: int, group_id: int, **kwargs) -> Dict[str, Any]:
        ...
        if group_id:
            try:
                ...
                data.update(
                    {
                        "QQ号": member_info.get("user_id"),
                        "昵称": member_info.get("nickname"),
                        "群名片": member_info.get("card") or member_info.get("nickname"),
                        "角色": {"owner": "群主", "admin": "管理员", "member": "成员"}.get(member_info.get("role"), "成员"),
                        "头衔": member_info.get("title") or "无",
                        "入群时间": datetime.fromtimestamp(member_info.get("join_time", 0)).strftime("%Y-%m-%d %H:%M"),
                        "最后发言": datetime.fromtimestamp(member_info.get("last_sent_time", 0)).strftime("%Y-%m-%d %H:%M"),
                        "等级": member_info.get("level"),
                    }
                )
            except Exception as exc:
                errors.append(f"群成员信息获取失败：{exc}")
            ...
            data.update(
                {
                    ...
                    "性别": {"male": "男", "female": "女", "unknown": "未知"}.get(user_info.get("sex"), "未知"),
                    "年龄": user_info.get("age"),
                }
            )
        except Exception as exc:
            errors.append(f"个人信息获取失败：{exc}")

        if not data:
            return {"success": False, "message": "；".join(errors) or "获取用户信息失败"}

        message = "执行成功"
        if errors:
            message = f"执行成功（部分信息未获取：{'；'.join(errors)}）"
