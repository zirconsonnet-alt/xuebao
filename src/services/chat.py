...
class ChatService(BaseService):
    ...
    settings_schema = [
        {"key": "auto_emoji_enabled", "title": "自动表情回复", "description": "控制机器人是否自动给消息点表情。", "type": "bool", "group": "互动开关"},
        {"key": "auto_like_enabled", "title": "自动点赞", "description": "控制机器人是否自动给好友点赞。", "type": "bool", "group": "互动开关"},
    ]
    ...
    @service_action(cmd="聊天互动服务")
    async def chat_service_menu(self):
        if not self.enabled:
            await self.group.send_msg("❌ 聊天互动服务未开启！")
            return
        routes = {
            "1": self.reset_liked_users,
        }
        flow = {
            "title": "欢迎使用聊天互动服务",
            "subtitle": "这个服务主要处理互动行为与轻量管理。",
            "text": (
                "请选择以下操作：\n"
                "1. 重置点赞列表\n\n"
                "输入【序号】或【指令】"
            ...
            "badges": [
                ...
                {"text": "互动管理", "tone": "success"},
            ],
            "sections": [
                {
                    "title": "可用操作",
                    "description": "当前服务没有复杂菜单，常用管理操作会直接执行。",
                    "columns": 1,
                    "items": [
                        {
                            "index": "1",
                            "title": "重置点赞列表",
                            "description": "清空今日已点赞用户记录。",
                            "meta": "回复 1 执行",
                            "status": "直达",
                            "status_tone": "accent",
                        }
