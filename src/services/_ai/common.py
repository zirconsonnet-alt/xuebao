...
_invalid_response_models: Dict[str, datetime] = {}
_invalid_response_failures: Dict[str, tuple[int, datetime]] = {}
_stream_only_models = {"Qwen/QwQ-32B"}
_default_excluded_tool_names = ("get_current_time",)
_INVALID_RESPONSE_COOLDOWN_SECONDS = 120
_INVALID_RESPONSE_FAILURE_WINDOW_SECONDS = 300
_INVALID_RESPONSE_FAILURE_THRESHOLD = 3
...
class InvalidModelResponseError(RuntimeError):
    def __init__(self, *, reason: str, response: Any = None):
        ...
        self.summary = summarize_invalid_model_response(response)
        message = self.reason
        if self.summary:
            message = f"{message}: {self.summary}"
...
def summarize_invalid_model_response(response: Any, limit: int = 240) -> str:
    if response is None:
        return ""
    text = " ".join(repr(response).split())
    if len(text) > limit:
        return text[: max(0, limit - 3)] + "..."
    return text


def mark_model_invalid_response(
    model: str,
    *,
    cooldown_seconds: int = _INVALID_RESPONSE_COOLDOWN_SECONDS,
) -> tuple[int, bool]:
    if not model:
        return 0, False
    ...
    previous = _invalid_response_failures.get(model)
    if previous:
        previous_count, previous_at = previous
        if now - previous_at <= timedelta(seconds=_INVALID_RESPONSE_FAILURE_WINDOW_SECONDS):
            count = previous_count

    count += 1
    _invalid_response_failures[model] = (count, now)

    if count >= _INVALID_RESPONSE_FAILURE_THRESHOLD:
        _invalid_response_models[model] = now + timedelta(seconds=max(1, int(cooldown_seconds)))
        _invalid_response_failures.pop(model, None)
        return count, True

    return count, False


def reset_model_invalid_response_state(model: str) -> None:
    if not model:
        return
    model = str(model)
    _invalid_response_models.pop(model, None)
    _invalid_response_failures.pop(model, None)


def is_model_in_invalid_response_cooldown(model: str) -> bool:
    model = str(model)
    expires_at = _invalid_response_models.get(model)
    if not expires_at:
        return False
...
def extract_completion_message(response: Any) -> Any:
    choices = getattr(response, "choices", None)
    if not choices:
        raise InvalidModelResponseError(reason="响应缺少 choices", response=response)

    try:
        first_choice = choices[0]
    except Exception as exc:
        raise InvalidModelResponseError(
            reason=f"响应 choices 无法读取首项: {exc}",
            response=response,
        ) from exc
    ...
    if message is None:
        raise InvalidModelResponseError(reason="响应缺少首条 message", response=response)
    ...
    if not has_content and not tool_calls:
        raise InvalidModelResponseError(
            reason="响应 message 同时缺少 content 和 tool_calls",
            response=response,
        )

    if isinstance(message, dict):
        return type(
            "CompletionMessageProxy",
            (),
            {
                "content": content,
