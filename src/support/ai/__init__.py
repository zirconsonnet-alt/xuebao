...
重要入口：
- 运行配置：`config/ai_support.json`
- 密钥配置：`config/ai_support_secrets.json`
- 模型列表：`config/ai_model_chains.json`

如果你现在关心的是 LLM / 视觉 / 画图模型列表，请优先看
`src/support/ai/model_chains.py` 和 `config/ai_model_chains.json`。
"""
...
from .config import load_ai_runtime_config, load_ai_secrets_config
from .model_chains import (
...
class AIAssistantConfig:
    ...
    fetch_answers_cookie: str = ""
    llm_models: List[str] = field(default_factory=list)
    vision_models: List[str] = field(default_factory=list)
    image_generation_models: List[str] = field(default_factory=list)
    model: str = "deepseek-ai/DeepSeek-V3.2"
    official_llm_fallback_model: str = "deepseek-chat"
    ...
    tts_api_url: str = "http://117.50.252.57:8000"
    ...
    image_gen_model: str = "Tongyi-MAI/Z-Image-Turbo"
    image_gen_poll_interval: float = 2.0
    image_gen_max_wait: float = 120.0
    search_spider_dir: str = ""
    search_keywords_csv_path: str = ""
    search_result_csv_path: str = ""
    data_path: Path = Path("data") / "ai_assistant"
    voice_path: Path = Path("data") / "speech"
    music_path: Path = Path("data") / "bgm"

    @classmethod
    def load(cls) -> "AIAssistantConfig":
        runtime_config = load_ai_runtime_config()
        secrets_config = load_ai_secrets_config()
        model_chains = load_ai_model_chains()
        llm_models = list(model_chains.get("llm_models") or [])
        vision_models = list(model_chains.get("vision_models") or [])
        image_generation_models = list(model_chains.get("image_generation_models") or [])
        loaded = cls(
            api_key=str(secrets_config.get("api_key", "")),
            base_url=str(secrets_config.get("base_url", "https://api-inference.modelscope.cn/v1")),
            modelscope_api_key=str(secrets_config.get("modelscope_api_key", "")),
            modelscope_base_url=str(secrets_config.get("modelscope_base_url", "https://api-inference.modelscope.cn/v1")),
            anthropic_api_key=str(secrets_config.get("anthropic_api_key", "")),
            anthropic_base_url=str(secrets_config.get("anthropic_base_url", "")),
            deepseek_api_key=str(secrets_config.get("deepseek_api_key", "")),
            deepseek_base_url=str(secrets_config.get("deepseek_base_url", "https://api.deepseek.com")),
            fetch_answers_cookie=str(secrets_config.get("fetch_answers_cookie", "")),
            llm_models=llm_models,
            vision_models=vision_models,
            image_generation_models=image_generation_models,
            model=str(llm_models[0] if llm_models else "deepseek-ai/DeepSeek-V3.2"),
            official_llm_fallback_model=str(runtime_config.get("official_llm_fallback_model", "deepseek-chat")).strip(),
            default_temperature=float(runtime_config.get("default_temperature", 1.0)),
            tool_max_rounds=int(runtime_config.get("tool_max_rounds", 3)),
            tool_orchestrator=str(runtime_config.get("tool_orchestrator", "langgraph")),
            mcp_servers=list(runtime_config.get("mcp_servers", [])),
            max_msg_count=int(runtime_config.get("max_msg_count", 40)),
            max_total_length=int(runtime_config.get("max_total_length", 32000)),
            max_single_msg_length=int(runtime_config.get("max_single_msg_length", 4000)),
            default_rate_limit_per_hour=int(runtime_config.get("default_rate_limit_per_hour", 3)),
            default_rate_limit_enabled=bool(runtime_config.get("default_rate_limit_enabled", True)),
            default_rate_limit_warning=str(runtime_config.get("default_rate_limit_warning", cls.default_rate_limit_warning)),
            default_redirect_group=int(runtime_config.get("default_redirect_group", 1034063784)),
            tts_api_url=str(runtime_config.get("tts_api_url", "http://117.50.252.57:8000")),
            tts_local_service_url=str(runtime_config.get("tts_local_service_url", "http://127.0.0.1:3005")),
            vision_model=str(vision_models[0] if vision_models else "moonshotai/Kimi-K2.5"),
            vision_default_prompt=str(runtime_config.get("vision_default_prompt", "请简洁描述这张图片的内容，不超过100字。")),
            image_gen_model=str(image_generation_models[0] if image_generation_models else "Tongyi-MAI/Z-Image-Turbo"),
            image_gen_poll_interval=float(runtime_config.get("image_gen_poll_interval", 2.0)),
            image_gen_max_wait=float(runtime_config.get("image_gen_max_wait", 120.0)),
            search_spider_dir=str(runtime_config.get("search_spider_dir", "")),
            search_keywords_csv_path=str(runtime_config.get("search_keywords_csv_path", "")),
            search_result_csv_path=str(runtime_config.get("search_result_csv_path", "")),
            data_path=Path(str(runtime_config.get("data_path", "data/ai_assistant"))),
            voice_path=Path(str(runtime_config.get("voice_path", "data/speech"))),
            music_path=Path(str(runtime_config.get("music_path", "data/bgm"))),
        )
    ...
    def __post_init__(self):
        ...
        self.modelscope_base_url = self._normalize_base_url(self.modelscope_base_url)
        self.deepseek_base_url = self._normalize_base_url(self.deepseek_base_url)
        if self.anthropic_base_url:
            self.anthropic_base_url = self._normalize_base_url(self.anthropic_base_url)
...
def _get_modelscope_upstream() -> tuple[str, str]:
    return config.modelscope_api_key, config.modelscope_base_url


def _get_anthropic_upstream() -> tuple[str, str]:
    return config.anthropic_api_key, config.anthropic_base_url


def _get_deepseek_upstream() -> tuple[str, str]:
    return config.deepseek_api_key, config.deepseek_base_url


def _has_deepseek_upstream() -> bool:
    api_key, base_url = _get_deepseek_upstream()
...
def get_llm_models_to_try(primary_model: str | None = None) -> List[str]:
    return get_llm_model_chain(primary_model)


def get_official_llm_fallback_model() -> str:
    return str(config.official_llm_fallback_model or "").strip()


def has_official_llm_fallback_upstream() -> bool:
    fallback_model = get_official_llm_fallback_model()
    return bool(
        fallback_model
        and fallback_model.startswith("deepseek-")
        and _has_deepseek_upstream()
    )


def get_official_llm_fallback_upstream_context(model: str | None = None) -> UpstreamContext:
    fallback_model = str(model or get_official_llm_fallback_model() or "").strip()
    if not fallback_model:
        raise ValueError("未配置 DeepSeek 官方兜底模型")
    if not fallback_model.startswith("deepseek-"):
        raise ValueError(f"官方兜底模型必须是 DeepSeek 官方模型，当前为：{fallback_model}")

    api_key, base_url = _get_deepseek_upstream()
    if not api_key or not base_url:
        raise ValueError("未配置 DeepSeek 官方上游")

    normalized_base_url = AIAssistantConfig._normalize_base_url(base_url)
    headers = {
...
def get_vision_models_to_try() -> List[str]:
    return get_vision_model_chain()


def get_image_generation_models_to_try() -> List[str]:
    return get_image_generation_model_chain()
...
class SpeechGenerator(ABC):
    ...
    async def gen_speech(self, text, voice_id, music_enable=False):
        ...
        if result_path and Path(self.voice_path) == Path("data") / "speech":
            cleanup_speech_cache(protected_paths=[result_path])
        return result_path
        ...
        async with aiohttp.ClientSession() as session:
            for model_name in get_vision_models_to_try():
                ...
                try:
                    ...
                    last_error = "图片识别超时，请稍后再试"
                except Exception as exc:
                    traceback.print_exc()
                    last_error = f"图片识别失败: {exc}"
        ...
        async with aiohttp.ClientSession() as session:
            for model_name in get_vision_models_to_try():
                ...
                try:
                    ...
                    last_error = "视频识别超时，请稍后再试"
                except Exception as exc:
                    traceback.print_exc()
                    last_error = f"视频识别失败: {exc}"
